...
class StereoAnchorNode(Node):
    ...
    def __init__(self, cfg: StereoConfig | None = None):
        ...
        if cfg is not None:
            ...
            self.cfg = StereoConfig(
                ...
                floaters=self.declare_parameter(
                    "floaters", ["cam_low", "cam_side"]
                ).value,
                atf=self.declare_parameter("atf", "/anchor_tracker/pose").value,
                ftf=self.declare_parameter(
                    "ftf", ["/cam_low/pose", "/cam_side/pose"]
                ).value,
        ...
        # anchor -> atf pose
        self.latest_ap: Optional[PoseStamped] = None
    ...
    def _tick(self):
        ...
        print(
            {
                floater: (floater, topic)
                for floater, topic in zip(self.cfg.floaters, self.cfg.ftf)
            }
        )

        self.fps = {
            floater: self.safe_lookup(floater, topic)
            for floater, topic in zip(self.cfg.floaters, self.cfg.ftf)
        }
        print(self.fps)
        ...
        for floater, fp_pose in self.fps.items():
            ...
            Rz = euler2mat(0.0, 0.0, np.pi)  # 180° about Z
