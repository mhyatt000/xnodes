...
class Base(Node):
    """
    Base class for ROS2 nodes imported from xgym (deprecated) package.
    Provides common helper functionality such as logging, timing, and camera subscription management.
    """
    ...
    def set_active(self, msg: Bool):
        """
        Sets the active state of the node based on incoming Bool message.
        self.active can be used to control whether the node should process data or remain idle.
        """
    ...
    def logp(self, desc):
        """
        Logs a message at intervals defined by self.hz.
        reduces spam in the logs by only logging every 1/self.hz calls.
        usually logs once per second if self.logp is called at self.hz frequency.
        """
        if self.p % self.hz == 0:
