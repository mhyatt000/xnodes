...
import sys
import math
from .update import ReprojectionErrorKnotSequenceUpdateStrategy
...
class RsCalibratorConfiguration:
    deltaX = 1e-8
...
class RsCalibrator:
        ...
        if self.__config.adaptiveKnotPlacement:
            ...
            for iteration in range(self.__config.maxKnotPlacementIterations):
                ...
                self.__poseSpline = knotUpdateStrategy.getUpdatedSpline(
                    self.__poseSpline_dv.spline(), knots, self.__config.splineOrder
                )
    ...
    def __generateExtrinsicsInitialGuess(self):
        ...
        for idx, observation in enumerate(self.__observations):
            ...
            if success:
                ...
                sm.logWarn(
                    "Could not estimate T_t_c for observation at index {0}".format(idx)
                )
    ...
    def __generateIntrinsicsInitialGuess(self):
        ...
        if self.__isRollingShutter():
            ...
            self.__camera.shutter().setParameters(
                np.array([1.0 / self.__config.framerate / float(sensorRows)])
            )
    ...
    def __generateInitialSpline(
        self, splineOrder, timeOffsetPadding, numberOfKnots=None, framerate=None
    ):
        poseSpline = bsplines.BSplinePose(splineOrder, sm.RotationVector())

        # Get the observation times.
        times = np.array(
            [observation.time().toSec() for observation in self.__observations]
        )
        # get the pose values of the initial transformations at observation time
        curve = np.matrix(
            [
                poseSpline.transformationToCurveValue(observation.T_t_c().T())
    ...
    def __buildOptimizationProblem(self, W):
        ...
        problem.addDesignVariable(
            self.__camera_dv.shutterDesignVariable(), CALIBRATION_GROUP_ID
        )
        problem.addDesignVariable(
            self.__camera_dv.projectionDesignVariable(), CALIBRATION_GROUP_ID
        )
        problem.addDesignVariable(
            self.__camera_dv.distortionDesignVariable(), CALIBRATION_GROUP_ID
        )
    ...
    def __buildErrorTerm(self, frame, keypoint_index, p_t, camera_dv, poseSpline_dv):
        ...
        if self.__isRollingShutter():
            return self.__cameraModelFactory.reprojectionErrorAdaptiveCovariance(
                frame, keypoint_index, p_t, camera_dv, poseSpline_dv
            )
        else:
            return self.__cameraModelFactory.reprojectionError(
                frame, keypoint_index, p_t, camera_dv
            )
