...
class ReprojectionErrorKnotSequenceUpdateStrategy:
    ...
    def generateKnotList(self, reprojection_errors, poseSpline):
        ...
        [errorTermsPerSegment, errorPerSegment] = self.__getErrorPerSegment(
            times, errors, knots
        )

        disabledTimeSegments = self.__removeSegmentsWithoutImprovement(
            times, errors, self.__disabledTimeSegments
        )

        [filteredKnots, disabledTimeSegments] = (
            self.__removeSegmentsWithoutObservations(
                knots, errorPerSegment, disabledTimeSegments
            )
        )

        [errorTermsPerSegmentFiltered, errorPerSegmentFiltered] = (
            self.__getErrorPerSegment(times, errors, filteredKnots)
        )

        updatedKnots = self.__generateKnotSequence(
            errorPerSegmentFiltered,
    ...
    def getUpdatedSpline(self, poseSpline, knots, splineOrder):
        ...
        newPoseSpline.initPoseSplineSparseKnots(
            times, splinePoses, np.array(knots), 1e-6
        )
    ...
    def __getErrorAndTimestamp(self, reprojection_errors):
        ...
        newIdx = sorted(list(range(len(times))), key=times.__getitem__)
    ...
    def __removeSegmentsWithoutObservations(
        self, knots, errorPerSegment, disabledTimeSegments=[]
    ):
        filteredKnots = []
        ...
        for i, error in enumerate(errorPerSegment):
            if (
                p_error == 0 and error == 0 and i > 6 and i < len(errorPerSegment) - 6
    ...
    def __generateKnotSequence(
        ...
        for i, error in enumerate(errorPerSegmentFiltered):
            ...
            if error > expectedNormalError and i < numberOfKnots - 1:
                ...
                elif disabledTimeSegments is not None and self.__isSegmentDisabled(
                    disabledTimeSegments, newKnot
                ):
                    newKnots.append(knots[i])
                else:
                    # split:
    ...
    def __time2KnotSection(self, t, knots, segment):
        i = segment[
            0
        ]  # we assume that the times are ordered thus we do not need to check before the previous segment
        if i == -1:
            i = 0
        while i < len(knots) - 1:
    ...
    def __removeSegmentsWithoutImprovement(self, times, errors, disabledTimeSegments):
        ...
        if (
            self.__previousKnotSequence is not None
            and self.__previousErrorTerms is not None
        ):
            timeSegments = []
            ...
            for i, t in enumerate(times):
                segment = self.__time2KnotSection(
                    t, self.__previousKnotSequence, segment
                )
                errorPerOldSegment[segment[0]] += errors[i]
                timeSegments.append(
                    (
                        self.__previousKnotSequence[segment[0]],
                        self.__previousKnotSequence[segment[1]],
                    )
                )
