...
class BagImuDatasetReader(object):
    def __init__(
        self, bagfile, imutopic, bag_from_to=None, perform_synchronization=False
    ):
        ...
        if imutopic is None:
            raise RuntimeError(
                "Please pass in a topic name referring to the imu stream in the bag file\n{0}".format(
                    self.bag
                )
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Could not find topic {0} in {1}.".format(imutopic, self.bagfile)
            )
    ...
    def truncateIndicesFromTime(self, indices, bag_from_to):
        ...
        sm.logWarn(
            "BagImuDatasetReader: truncated {0} / {1} messages.".format(
