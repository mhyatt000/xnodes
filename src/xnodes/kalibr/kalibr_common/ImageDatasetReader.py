...
class BagImageDatasetReader(object):
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "Could not find topic {0} in {1}.".format(imagetopic, self.bagfile)
            )
        ...
        for idx in self.indices:
            ...
            if self.perform_synchronization:
                self.timestamp_corrector.correctTimestamp(
                    data.header.stamp.to_sec(), stamp.to_sec()
                )
    ...
    def truncateIndicesFromTime(self, indices, bag_from_to):
        ...
        if bag_from_to[1] > baglength:
            sm.logWarn(
                "Bag end time of {0} s is bigger than the total length of {1} s".format(
        ...
        sm.logWarn(
            "BagImageDatasetReader: truncated {0} / {1} images (from-to).".format(
    ...
    def truncateIndicesFromFreq(self, indices, freq):
        ...
        for idx in self.indices:
            ...
            if (timestamp - timestamp_last) >= 1.0 / freq:
                ...
                valid_indices.append(idx)
        sm.logWarn(
            "BagImageDatasetReader: truncated {0} / {1} images (frequency)".format(
                len(indices) - len(valid_indices), len(indices)
            )
        )
        return valid_indices
    ...
    def getImage(self, idx):
        ...
        if self.perform_synchronization:
            timestamp = acv.Time(
                self.timestamp_corrector.getLocalTime(data.header.stamp.to_sec())
            )
        else:
        ...
        if data._type == "mv_cameras/ImageSnappyMsg":
            ...
            img_data = np.reshape(
                self.uncompress(np.fromstring(data.data, dtype="uint8")),
            ...
            # https://github.com/ros-perception/image_pipeline/blob/6caf51bd4484ae846cd8a199f7a6a4b060c6373a/image_proc/src/libimage_proc/processor.cpp#L70
                ...
                raise RuntimeError(
                    ...
                    "bayer_rggb8, bayer_bggr8, bayer_gbrg8, bayer_grbg8".format(
                        data.encoding
                    )
                )
        else:
            raise RuntimeError(
                ...
                "mv_cameras/ImageSnappyMsg, sensor_msgs/CompressedImage, sensor_msgs/Image".format(
                    data._type
                )
            )
