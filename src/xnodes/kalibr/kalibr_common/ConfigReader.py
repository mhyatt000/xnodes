    ...
    def __init__(self, camera_model, intrinsics, dist_model, dist_coeff, resolution):
        ...
        if camera_model == "pinhole":
            ...
            if dist_model == "radtan":
                dist = cv.RadialTangentialDistortion(
                ...
                self.keypointType = cv.Keypoint2
                self.reprojectionErrorType = (
                    cvb.EquidistantDistortedPinholeReprojectionErrorSimple
                )
                ...
                self.keypointType = cv.Keypoint2
                self.reprojectionErrorType = (
                    cvb.FovDistortedPinholeReprojectionErrorSimple
                )
                self.undistorterType = cv.FovPinholeUndistorterNoMask
            elif dist_model == "none":
                proj = cv.PinholeProjection(
                    focalLength[0],
                    focalLength[1],
                    principalPoint[0],
                    principalPoint[1],
                ...
                self.raiseError(
                    "pinhole camera model does not support distortion model '{}'".format(
                        dist_model
                    )
                )
            ...
            if dist_model == "radtan":
                ...
                dist = cv.EquidistantPinholeProjection(
                    dist_coeff[0], dist_coeff[1], dist_coeff[2], dist_coeff[3]
                )
                ...
                self.keypointType = cv.Keypoint2
                self.reprojectionErrorType = (
                    cvb.EquidistantDistortedOmniReprojectionErrorSimple
                )

            elif dist_model == "none":
                proj = cv.OmniProjection(
                    xi_omni,
                    focalLength[0],
                    focalLength[1],
                    principalPoint[0],
                ...
                raise RuntimeError(
                    "omni camera model does not support distortion model '{}'".format(
                        dist_model
                    )
                )
            ...
            if dist_model == "none":
                ...
                raise RuntimeError(
                    "camera model {} does not support distortion model '{}'".format(
            ...
            if dist_model == "none":
                proj = cv.DoubleSphereProjection(
                    xi_ds,
                    alpha_ds,
                    focalLength[0],
                    focalLength[1],
...
def catch_keyerror(f):
    ...
    def func(*args, **kwargs):
        try:
            ...
            args[0].raiseError(
                "Field {0} missing in file: {1}".format(e, args[0].yamlFile)
            )
...
class ParametersBase(object):
    ...
    def readYaml(self):
        try:
            ...
            self.raiseError(
                "Could not read configuration from {0}".format(self.yamlFile)
            )
    ...
    def writeYaml(self, filename=None):
        ...
        try:
            with open(filename, "w") as outfile:
                outfile.write(
                    yaml.dump(self.data, default_flow_style=None, width=2147483647)
                )
        except:
            self.raiseError(
                "Could not write configuration to {0}".format(self.yamlFile)
            )
...
class CameraParameters(ParametersBase):
    ...
    def checkIntrinsics(self, model, intrinsics):
        cameraModels = ["pinhole", "omni", "eucm", "ds"]

        if model not in cameraModels:
            self.raiseError(
                "Unknown camera model '{}'; available models: {}.".format(
                    model, ", ".join(cameraModels)
                )
            )
        ...
        if model == "pinhole":
            ...
            if alpha_eucm < 0 or alpha_eucm >= 1:
                self.raiseError(
                    "invalid alpha_eucm of {} (0<=alpha<1)".format(alpha_eucm)
                )
            ...
            self.raiseError(
                "internal error: invalid camera model {} (should have been checked before)".format(
                    model
                )
            )

        if (
            not isinstance(focalLength[0], float)
            or not isinstance(focalLength[1], float)
            or focalLength[0] < 0.0
            or focalLength[1] < 0.0
        ):
            self.raiseError("invalid focalLength (2 floats)")

        if (
            principalPoint[0] < 0.0
            or principalPoint[1] < 0.0
            or not isinstance(principalPoint[0], float)
            or not isinstance(principalPoint[1], float)
        ):
            self.raiseError("invalid principalPoint")

    @catch_keyerror
    def getIntrinsics(self):
        self.checkIntrinsics(self.data["camera_model"], self.data["intrinsics"])
    ...
    def checkDistortion(self, model, coeffs):
        ...
        if model not in distortionModelsAndNumParams:
            self.raiseError(
                "Unknown distortion model '{}'. Supported models: {}. )".format(
        ...
        if len(coeffs) != distortionModelsAndNumParams[model]:
            self.raiseError(
                "distortion model '{}' requires {} coefficients; {} given".format(
...
class ImuParameters(ParametersBase):
    ...
    def getAccelerometerStatistics(self):
        self.checkAccelerometerStatistics(
            self.data["accelerometer_noise_density"],
            self.data["accelerometer_random_walk"],
        )
        accelUncertaintyDiscrete = self.data["accelerometer_noise_density"] / math.sqrt(
            1.0 / self.getUpdateRate()
        )
        return (
            accelUncertaintyDiscrete,
            self.data["accelerometer_random_walk"],
            self.data["accelerometer_noise_density"],
        )
    ...
    def getGyroStatistics(self):
        self.checkGyroStatistics(
            self.data["gyroscope_noise_density"], self.data["gyroscope_random_walk"]
        )
        gyroUncertaintyDiscrete = self.data["gyroscope_noise_density"] / math.sqrt(
            1.0 / self.getUpdateRate()
        )
        return (
            gyroUncertaintyDiscrete,
            self.data["gyroscope_random_walk"],
            self.data["gyroscope_noise_density"],
        )
    ...
    def printDetails(self, dest=sys.stdout):
        ...
        accelUncertaintyDiscrete, accelRandomWalk, accelUncertainty = (
            self.getAccelerometerStatistics()
        )
        gyroUncertaintyDiscrete, gyroRandomWalk, gyroUncertainty = (
            self.getGyroStatistics()
        )
        ...
        print(
            "    Noise density (discrete): {0} ".format(accelUncertaintyDiscrete),
            file=dest,
        )
        ...
        print(
            "    Noise density (discrete): {0} ".format(gyroUncertaintyDiscrete),
            file=dest,
        )
...
class CalibrationTargetParameters(ParametersBase):
    ...
    def checkTargetType(self, target_type):
        targetTypes = ["aprilgrid", "checkerboard", "circlegrid"]

        if target_type not in targetTypes:
            self.raiseError(
                "Unknown calibration target type. Supported types: {0}. )".format(
                    targetTypes
                )
            )
    ...
    def getTargetParams(self):
        ...
        if targetType == "checkerboard":
            try:
                ...
                self.raiseError(
                    "Calibration target configuration in {0} is missing the field: {1}".format(
                        self.yamlFile, str(e)
                    )
                )
    ...
    def printDetails(self, dest=sys.stdout):
        ...
        if targetType == "checkerboard":
            ...
            print(
                "    Distance: {0} [m]".format(targetParams["rowSpacingMeters"]),
                file=dest,
            )
            print("  Cols", file=dest)
            print("    Count: {0}".format(targetParams["targetCols"]), file=dest)
            print(
                "    Distance: {0} [m]".format(targetParams["colSpacingMeters"]),
                file=dest,
            )
        elif targetType == "aprilgrid":
...
class CameraChainParameters(ParametersBase):
    ...
    def setExtrinsicsLastCamToHere(self, camNr, extrinsics):
        if camNr == 0:
            raise RuntimeError(
                "setExtrinsicsLastCamToHere(): can't set extrinsics for first cam in chain (cam0=base)"
            )
        if not isinstance(extrinsics, sm.Transformation):
            raise RuntimeError(
                "setExtrinsicsLastCamToHere(): provide extrinsics as an sm.Transformation object"
            )
    ...
    def getExtrinsicsLastCamToHere(self, camNr):
        ...
        try:
            trafo = sm.Transformation(
                np.array(self.data["cam{0}".format(camNr)]["T_cn_cnm1"])
            )
            trafo.T()  # for error checking
        except:
            self.raiseError(
                "invalid camera baseline (cam{0} in {1})".format(camNr, self.yamlFile)
            )
        return trafo
    ...
    def setExtrinsicsImuToCam(self, camNr, extrinsics):
        if not isinstance(extrinsics, sm.Transformation):
            raise RuntimeError(
                "setExtrinsicsImuToCam(): provide extrinsics as an sm.Transformation object"
            )
    ...
    def getExtrinsicsImuToCam(self, camNr):
        ...
        try:
            trafo = sm.Transformation(
                np.array(self.data["cam{0}".format(camNr)]["T_cam_imu"])
            )
            trafo.T()  # for error checking
        except:
            self.raiseError(
                "invalid T_cam_imu (cam{0} in {1})".format(camNr, self.yamlFile)
            )
        return trafo
    ...
    def checkTimeshiftCamImu(self, camNr, time_shift):
        ...
        if not isinstance(time_shift, float):
            self.raiseError(
                "invalid timeshift cam-imu (cam{0} in {1})".format(camNr, self.yamlFile)
            )

    @catch_keyerror
    def getTimeshiftCamImu(self, camNr):
        self.checkTimeshiftCamImu(
            camNr, self.data["cam{0}".format(camNr)]["timeshift_cam_imu"]
        )
