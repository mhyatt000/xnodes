    ...
    def addObservation(self, cam_id, obs):
        ...
        if not timestamps_table:
            ...
            nearest_timestamp = min(
                timestamps_table, key=lambda x: abs(x - timestamp_obs)
            )
        ...
        if cam_id not in self.targetViews[timestamp]:
            ...
            self.targetViews[timestamp][cam_id]["observed_corners"] = set(
                obs.getCornersIdx()
            )
        else:
            # we already have a view from this camera on this timestamp --> STH IS WRONG
            sm.logError(
                "[TargetViewTable]: Tried to add second view to a given cameraId & "
