...
class CameraGeometry(object):
    def __init__(
        self, cameraModel, targetConfig, dataset, geometry=None, verbose=False
    ):
    ...
    def initGeometryFromObservations(self, observations):
        ...
        if not success:
            sm.logError(
                "initialization of focal length for cam with topic {0} failed  ".format(
                    self.dataset.topic
                )
            )
        ...
        if self.model == acvb.DistortedOmni:
            ...
            if not success:
                sm.logError(
                    "initialization of intrinsics for cam with topic {0} failed  ".format(
                        self.dataset.topic
                    )
                )
    ...
    def fromTargetViewObservations(
        ...
        for i in range(0, rval.dv_T_target_camera.numDesignVariables()):
            rval.addDesignVariable(
                rval.dv_T_target_camera.getDesignVariable(i), TRANSFORMATION_GROUP_ID
            )
        ...
        for baseline_dv in baselines:
            for i in range(0, baseline_dv.numDesignVariables()):
                rval.addDesignVariable(
                    baseline_dv.getDesignVariable(i), CALIBRATION_GROUP_ID
                )
...
def removeCornersFromBatch(
    batch, camId_cornerIdList_tuples, useBlakeZissermanMest=True
):
...
class CameraCalibration(object):
    def __init__(
        ...
        self.target = CalibrationTarget(
            cameras[0].ctarget.detector.target(), estimateLandmarks=estimateLandmarks
        )
    ...
    def addTargetView(self, timestamp, rig_observations, T_tc_guess, force=False):
        ...
        if (
            self.estimator_return_value.numIterations
            >= self.optimizerOptions.maxIterations
        ):
