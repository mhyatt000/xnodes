...
class MulticamCalibrationGraph(object):
    ...
    def getInitialGuesses(self, cameras):
        ...
        outdegrees.index(min(outdegrees))
        ...
        for baseline_edge_id in self.optimal_baseline_edges:
            ...
            success, baseline_HL = kcc.stereoCalibrate(
                cameras[camL_nr], cameras[camH_nr], obs_list, distortionActive=False
            )

            if success:
                sm.logDebug(
                    "baseline_{0}_{1}={2}".format(camL_nr, camH_nr, baseline_HL.T())
                )
            else:
                sm.logError(
                ...
                sm.logError(
                    "estimated baseline_{0}_{1}={2}".format(
        ...
        for baseline_id in range(0, self.numCams - 1):
            ...
            path = G_optimal_baselines.get_shortest_paths(baseline_id, baseline_id + 1)[
                0
            ]
            ...
            for path_idx in range(0, len(path) - 1):
                ...
                T_edge = self.G.es[self.G.get_eid(source_vert, target_vert)][
                    "baseline_HL"
                ]
    ...
    def getTargetPoseGuess(self, timestamp, cameras, baselines_HL=[]):
        ...
        if not success:
            sm.logWarn(
                "getTargetPoseGuess: solvePnP failed with solution: {0}".format(T_t_cN)
            )
    ...
    def getAllMutualObsBetweenTwoCams(self, camA_nr, camB_nr):
        ...
        try:
            ...
            sm.logError(
                "getAllMutualObsBetweenTwoCams: no mutual observations between the two cams!"
            )
            return [], []
