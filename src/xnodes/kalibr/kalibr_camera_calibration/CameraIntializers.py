...
def stereoCalibrate(
    camL_geometry, camH_geometry, obslist, distortionActive=False, baseline=None
):
    #####################################################
    ...
    for cidx, cam in enumerate([camL_geometry, camH_geometry]):
        sm.logDebug(
            "stereoCalibration: adding camera error terms for {0} calibration targets".format(
                len(obslist)
            )
        )
        ...
        sm.logDebug(
            "stereoCalibrate: added {0} camera error terms".format(
    ...
    if sm.getLoggingLevel() == sm.LoggingLevel.Debug:
        ...
        sm.logDebug(
            " Reprojection error squarred (camL):  mean {0}, median {1}, std: {2}".format(
                np.mean(e2), np.median(e2), np.std(e2)
            )
        )
        e2 = np.array([e.evaluateError() for e in reprojectionErrors1])
        sm.logDebug(
            " Reprojection error squarred (camH):  mean {0}, median {1}, std: {2}".format(
                np.mean(e2), np.median(e2), np.std(e2)
            )
        )
...
def calibrateIntrinsics(
    cam_geometry, obslist, distortionActive=True, intrinsicsActive=True
):
    # verbose output
    if sm.getLoggingLevel() == sm.LoggingLevel.Debug:
    ...
    reprojectionErrors = []
    sm.logDebug(
        "calibrateIntrinsics: adding camera error terms for {0} calibration targets".format(
            len(obslist)
        )
    )
    target_pose_dvs = list()
    ...
    sm.logDebug(
        "calibrateIntrinsics: added {0} camera error terms".format(
