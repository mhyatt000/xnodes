...
def plotPolarError(
    cself, cam_id, fno=1, clearFigure=True, stats=None, noShow=False, title=""
):
    if stats is None:
        stats = getAllPointStatistics(cself, cam_id)
    angleError = np.array(
        [[np.degrees(s.polarAngle), math.sqrt(s.squaredError)] for s in stats]
    )
    # sort by polar angle
...
def plotAzumithalError(
    cself, cam_id, fno=1, clearFigure=True, stats=None, noShow=False, title=""
):
    if stats is None:
        stats = getAllPointStatistics(cself, cam_id)
    angleError = np.array(
        [[np.degrees(s.azumithalAngle), math.sqrt(s.squaredError)] for s in stats]
    )
...
def plotAllReprojectionErrors(
    cself, cam_id, fno=1, noShow=False, clearFigure=True, title=""
):
    ...
    resolution = (
        cself.cameras[cam_id].geometry.projection().ru(),
...
def plotCornersAndReprojection(
    gridobs,
    reprojs,
    fno=1,
    cornerlist=None,
    ...
    plotCorners(
        gridobs, fno=fno, clearFigure=False, plotImage=True, cornerlist=cornerlist
    )
...
def recoverCovariance(cself):
    ...
    for cidx, cam in enumerate(cself.cameras):
        nt = (
            cam.geometry.minimalDimensionsDistortion()
    ...
    for cam_id, cam in enumerate(cself.cameras):
        ...
        if cameraModel == "omni":
            camParams.setIntrinsics(
                cameraModel, [P.xi(), P.fu(), P.fv(), P.cu(), P.cv()]
            )
        elif cameraModel == "pinhole":
            camParams.setIntrinsics(cameraModel, [P.fu(), P.fv(), P.cu(), P.cv()])
        elif cameraModel == "eucm":
            camParams.setIntrinsics(
                cameraModel, [P.alpha(), P.beta(), P.fu(), P.fv(), P.cu(), P.cv()]
            )
        elif cameraModel == "ds":
            camParams.setIntrinsics(
                cameraModel, [P.xi(), P.alpha(), P.fu(), P.fv(), P.cu(), P.cv()]
            )
        else:
...
def plotTrajectory(cself, fno=1, clearFigure=True, title=""):
    ...
    for view in views:
        ...
        if T_target_camera_last is not None:
    ...
    a3d.auto_scale_xyz(
        [traj_min[0] - size, traj_max[0] + size],
        [traj_min[1] - size, traj_max[1] + size],
        [traj_min[2] - size, traj_max[2] + size],
    )
...
def plotCameraRig(baselines, fno=1, clearFigure=True, title=""):
    ...
    for i, camera_frame in enumerate(camera_frames):
        sm.plotCoordinateFrame(
            a3d, camera_frame.T(), size=size, name="cam{0}".format(i)
        )
...
def exportPoses(cself, filename):
    ...
    for view in views:
        ...
        print(
            "{:.0f},".format(1e9 * view.timestamp)
            + ",".join(map("{:.6f}".format, position))
            + ","
            + ",".join(map("{:.6f}".format, orientation)),
            file=f,
        )
