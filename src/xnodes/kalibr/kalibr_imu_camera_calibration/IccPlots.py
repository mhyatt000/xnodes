...
def plotIMURates(cself, iidx, fno=1, clearFigure=True, noShow=False):
    ...
    pl.text(
        0.1,
        0.9,
        "avg dt (" + unit + ") = {:.2f} +- {:.4f}".format(rate_avg, rate_std),
        fontsize=12,
        transform=f.gca().transAxes,
    )
    pl.grid("on")
    pl.xlabel("time (s)")
...
def plotGyroError(cself, iidx, fno=1, clearFigure=True, noShow=False):
    errors = np.array(
        [np.dot(re.error(), re.error()) for re in cself.ImuList[iidx].gyroErrors]
    )
...
def plotGyroErrorPerAxis(cself, iidx, fno=1, clearFigure=True, noShow=False):
    ...
    for i in range(3):
        ...
        pl.plot(np.array([0.0, errors.shape[0]]), sigma * 3.0 * np.ones(2), "r--")
        pl.plot(np.array([0.0, errors.shape[0]]), -sigma * 3.0 * np.ones(2), "r--")
        pl.xlim([0.0, errors.shape[0]])


def plotAccelError(cself, iidx, fno=1, clearFigure=True, noShow=False):
    errors = np.array(
        [np.dot(re.error(), re.error()) for re in cself.ImuList[iidx].accelErrors]
    )
...
def plotAccelErrorPerAxis(cself, iidx, fno=1, clearFigure=True, noShow=False):
    ...
    for i in range(3):
        ...
        pl.plot(np.array([0, errors.shape[0]]), sigma * 3.0 * np.ones(2), "r--")
        pl.plot(np.array([0, errors.shape[0]]), -sigma * 3.0 * np.ones(2), "r--")
        pl.xlim([0.0, errors.shape[0]])
...
def plotAccelBias(cself, imu_idx, fno=1, clearFigure=True, noShow=False):
    ...
    times = np.array(
        [
            im.stamp.toSec()
            for im in imu.imuData
            if im.stamp.toSec() > bias.t_min() and im.stamp.toSec() < bias.t_max()
        ]
    )
    ...
    bounds = 3.0 * sigma_rw * np.sqrt(times)
    for i in range(3):
        pl.subplot(3, 1, i + 1)
...
def plotAngularVelocities(cself, iidx, fno=1, clearFigure=True, noShow=False):
    ...
    predictedAng_body = np.array(
        [err.getPredictedMeasurement() for err in imu.gyroErrors]
    ).T
...
def plotAccelerations(cself, iidx, fno=1, clearFigure=True, noShow=False):
    ...
    predicetedAccel_body = np.array(
        [err.getPredictedMeasurement() for err in imu.accelErrors]
    ).T
...
def plotReprojectionScatter(
    cself, cam_id, fno=1, clearFigure=True, noShow=False, title=""
):
    ...
    uncertainty_bound = pl.Circle(
        (0, 0),
        3.0 * cam.cornerUncertainty,
        color="k",
        linestyle="dashed",
        fill=False,
        lw=2,
        zorder=len(cam.allReprojectionErrors),
    )
...
class CameraPlot:
    ...
    def plot3Dgrid(self):
        ...
        for i in range(0, len(self.targetPoints)):
            self.ax.scatter(
                self.targetPoints[i, 0],
                self.targetPoints[i, 1],
                self.targetPoints[i, 2],
                color="g",
                s=1,
            )
    ...
    def plot3DCamera(self, T):
        ...
        if not self.initialized:
            self.cam_x = self.ax.plot(
                np.squeeze([ori[0], v1[0]]),
                np.squeeze([ori[1], v1[1]]),
                np.squeeze([ori[2], v1[2]]),
                color="r",
            )
            self.cam_y = self.ax.plot(
                np.squeeze([ori[0], v2[0]]),
                np.squeeze([ori[1], v2[1]]),
                np.squeeze([ori[2], v2[2]]),
                color="g",
            )
            self.cam_z = self.ax.plot(
                np.squeeze([ori[0], v3[0]]),
                np.squeeze([ori[1], v3[1]]),
                np.squeeze([ori[2], v3[2]]),
                color="b",
            )
