...
def printErrorStatistics(cself, dest=sys.stdout):
        ...
        if len(cam.allReprojectionErrors) > 0:
            ...
            print(
                "Reprojection error (cam{0}):     mean {1}, median {2}, std: {3}".format(
                    cidx, np.mean(e2), np.median(e2), np.std(e2)
                ),
                file=dest,
            )
        else:
    ...
    for iidx, imu in enumerate(cself.ImuList):
        ...
        print(
            "Gyroscope error (imu{0}):        mean {1}, median {2}, std: {3}".format(
                iidx, np.mean(e2), np.median(e2), np.std(e2)
            ),
            file=dest,
        )
        ...
        print(
            "Accelerometer error (imu{0}):    mean {1}, median {2}, std: {3}".format(
                iidx, np.mean(e2), np.median(e2), np.std(e2)
            ),
            file=dest,
        )
    ...
    for cidx, cam in enumerate(cself.CameraChain.camList):
        if len(cam.allReprojectionErrors) > 0:
            ...
            print(
                "Reprojection error (cam{0}) [px]:     no corners".format(cidx),
                file=dest,
            )
    ...
    for iidx, imu in enumerate(cself.ImuList):
        ...
        print(
            "Gyroscope error (imu{0}) [rad/s]:     mean {1}, median {2}, std: {3}".format(
                iidx, np.mean(e2), np.median(e2), np.std(e2)
            ),
            file=dest,
        )
        ...
        print(
            "Accelerometer error (imu{0}) [m/s^2]: mean {1}, median {2}, std: {3}".format(
                iidx, np.mean(e2), np.median(e2), np.std(e2)
            ),
            file=dest,
        )
...
def printBaselines(self):
    ...
    if nCams > 1:
        for camNr in range(0, nCams - 1):
            ...
            print("")
            print(
                "Baseline (cam{0} to cam{1}): [m] {2}".format(camNr, camNr + 1, isFixed)
            )
            print(T.T())
            print(baseline, "[m]")
    ...
    if cself.CameraChain:
        for cidx, cam in enumerate(cself.CameraChain.camList):
            ...
            plots.plotReprojectionScatter(
                cself, cidx, fno=f.number, noShow=True, title=title
            )
            plotter.add_figure(title, f)
            figs.append(f)
...
def printResultTxt(cself, stream=sys.stdout):
    ...
    for camNr in range(0, nCams):
        ...
        print("", file=stream)
        print(
            "timeshift cam{0} to imu0: [s] (t_imu = t_cam + shift)".format(camNr),
            file=stream,
        )
