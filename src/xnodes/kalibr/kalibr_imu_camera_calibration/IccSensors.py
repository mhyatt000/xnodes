...
def initCameraBagDataset(bagfile, topic, from_to, freq, perform_synchronization):
    ...
    reader = kc.BagImageDatasetReader(
        bagfile,
        topic,
        bag_from_to=from_to,
        bag_freq=freq,
...
class IccCamera:
    def __init__(
        ...
        self.setupCalibrationTarget(
            targetConfig,
            showExtraction=showCorners,
            showReproj=showReproj,
            imageStepping=showOneStep,
        )
        multithreading = not (showCorners or showReproj or showOneStep)
        self.targetObservations = kc.extractCornersFromDataset(
            self.dataset, self.detector, multithreading=multithreading
        )

        # an estimate of the gravity in the world coordinate frame
        self.gravity_w = np.array([9.80655, 0.0, 0.0])

    def setupCalibrationTarget(
        self, targetConfig, showExtraction=False, showReproj=False, imageStepping=False
    ):
    ...
    def findOrientationPriorCameraToImu(self, imu):
        ...
        options.linearSolver = (
            aopt.BlockCholeskyLinearSystemSolver()
        ...
        imu.GyroBiasPrior = (
            (imu.GyroBiasPriorCount - 1.0) / imu.GyroBiasPriorCount * imu.GyroBiasPrior
    ...
    def initPoseSplineFromCamera(
        self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02
    ):
        ...
        print("")
        print(
            "Initializing a pose spline with %d knots (%f knots per second over %f seconds)"
    ...
    def addDesignVariables(
        ...
        self.T_c_b_Dv = aopt.TransformationDv(
            self.T_extrinsic, rotationActive=active, translationActive=active
        )
        for i in range(0, self.T_c_b_Dv.numDesignVariables()):
            problem.addDesignVariable(
                self.T_c_b_Dv.getDesignVariable(i), baselinedv_group_id
            )
    ...
    def addCameraErrorTerms(
        self, problem, poseSplineDv, T_cN_b, blakeZissermanDf=0.0, timeOffsetPadding=0.0
    ):
        print("")
        ...
        for obs in self.targetObservations:
            ...
            if (
                frameTimeScalar <= poseSplineDv.spline().t_min()
        ...
        print(
            "\r  Added {0} camera error terms                      ".format(
...
class IccCameraChain:
    def __init__(self, chainConfig, targetConfig, parsed):
        ...
        for camNr in range(0, chainConfig.numCameras()):
            ...
            dataset = initCameraBagDataset(
                parsed.bagfile[0],
                camConfig.getRosTopic(),
                parsed.bag_from_to,
                parsed.bag_freq,
    ...
    def initializeBaselines(self):
        ...
        for camNr in range(1, len(self.camList)):
            self.camList[
                camNr
            ].T_extrinsic = self.chainConfig.getExtrinsicsLastCamToHere(camNr)

            print("Baseline between cam{0} and cam{1} set to:".format(camNr - 1, camNr))
            print("T= ", self.camList[camNr].T_extrinsic.T())
            print(
                "Baseline: ",
                np.linalg.norm(self.camList[camNr].T_extrinsic.t()),
                " [m]",
            )

    # initialize a pose spline for the chain
    def initializePoseSplineFromCameraChain(
        self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02
    ):
    ...
    def getResultTimeShift(self, camNr):
        return (
            self.camList[camNr].cameraTimeToImuTimeDv.toScalar()
            + self.camList[camNr].timeshiftCamToImuPrior
        )

    def addDesignVariables(
        self, problem, noTimeCalibration=True, noChainExtrinsics=True
    ):
    ...
    def addCameraChainErrorTerms(
        self, problem, poseSplineDv, blakeZissermanDf=-1, timeOffsetPadding=0.0
    ):
        ...
        for camNr, cam in enumerate(self.camList):
            ...
            # add the error terms
            cam.addCameraErrorTerms(
                problem, poseSplineDv, T_cN_b, blakeZissermanDf, timeOffsetPadding
            )
...
class IccImu(object):
    class ImuParameters(kc.ImuParameters):
        ...
        def printDetails(self, dest=sys.stdout):
            ...
            print(
                "  time offset with respect to IMU0: {0} [s]".format(
                    self.data["time_offset"]
                ),
                file=dest,
            )
    ...
    def __init__(
        self, imuConfig, parsed, isReferenceImu=True, estimateTimedelay=True, imuNr=0
    ):
        ...
        # load dataset
        self.dataset = initImuBagDataset(
            parsed.bagfile[0],
        ...
        # statistics
        self.accelUncertaintyDiscrete, self.accelRandomWalk, self.accelUncertainty = (
            self.imuConfig.getAccelerometerStatistics()
        )
        self.gyroUncertaintyDiscrete, self.gyroRandomWalk, self.gyroUncertainty = (
            self.imuConfig.getGyroStatistics()
        )
        ...
        self.q_i_b_prior = np.array([0.0, 0.0, 0.0, 1.0])
        self.timeOffset = 0.0
    ...
    def loadImuData(self):
        ...
        Raccel = (
            np.eye(3) * self.accelUncertaintyDiscrete * self.accelUncertaintyDiscrete
        )
        ...
        if len(self.imuData) > 1:
            print(
                "\r  Read %d imu readings over %.1f seconds                   "
    ...
    def addDesignVariables(self, problem):
        ...
        addSplineDesignVariables(
            problem, self.gyroBiasDv, setActive=True, group_id=HELPER_GROUP_ID
        )
        addSplineDesignVariables(
            problem, self.accelBiasDv, setActive=True, group_id=HELPER_GROUP_ID
        )
        ...
        self.r_b_Dv = aopt.EuclideanPointDv(np.array([0.0, 0.0, 0.0]))
    ...
    def addAccelerometerErrorTerms(
        self, problem, poseSplineDv, g_w, mSigma=0.0, accelNoiseScale=1.0
    ):
        print("")
        ...
        for im in self.imuData:
            ...
            if (
                tk > poseSplineDv.spline().t_min()
                and tk < poseSplineDv.spline().t_max()
            ):
        ...
        print(
            "\r  Added {0} of {1} accelerometer error terms (skipped {2} out-of-bounds measurements)".format(
                len(self.imuData) - num_skipped, len(self.imuData), num_skipped
            )
        )
        self.accelErrors = accelErrors

    def addGyroscopeErrorTerms(
        self, problem, poseSplineDv, mSigma=0.0, gyroNoiseScale=1.0, g_w=None
    ):
        print("")
        ...
        print(
            "\r  Added {0} of {1} gyroscope error terms (skipped {2} out-of-bounds measurements)".format(
                len(self.imuData) - num_skipped, len(self.imuData), num_skipped
            )
        )
    ...
    def addBiasMotionTerms(self, problem):
        ...
        accelBiasMotionErr = asp.BSplineEuclideanMotionError(
            self.accelBiasDv, Waccel, 1
        )
    ...
    def getTransformationFromBodyToImu(self):
        ...
        return sm.Transformation(
            sm.r2quat(self.q_i_b_Dv.toRotationMatrix()),
    ...
    def findOrientationPrior(self, referenceImu):
        ...
        angularVelocity.initConstantSpline(
            startTime, endTime, knots, np.array([0.0, 0.0, 0.0])
        )
        ...
        def referenceAbsoluteOmega(dt=np.array([0.0])):
            return np.asarray(
                [
                    np.linalg.norm(
                        angularVelocityDv.toEuclidean(im.stamp.toSec() + dt[0], 0)
                    )
                    for im in self.imuData
                    if (
                        im.stamp.toSec() + dt[0] > angularVelocity.t_min()
                        and im.stamp.toSec() + dt[0] < angularVelocity.t_max()
                    )
                ]
            )

        def absoluteOmega(dt=np.array([0.0])):
            return np.asarray(
                [
                    np.linalg.norm(im.omega)
                    for im in self.imuData
                    if (
                        im.stamp.toSec() + dt[0] > angularVelocity.t_min()
                        and im.stamp.toSec() + dt[0] < angularVelocity.t_max()
        ...
        if self.estimateTimedelay and not self.isReferenceImu:
            ...
            def objectiveFunction(dt):
                return (
                    np.linalg.norm(referenceAbsoluteOmega(dt) - absoluteOmega(dt)) ** 2
                )

            refined_shift = scipy.optimize.fmin(
                objectiveFunction, np.array([shift]), maxiter=100
            )[0]
        ...
        print(
            ...
            (
                ""
                if self.estimateTimedelay
                else " (this offset is not accounted for in the calibration)"
            ),
        )
...
class IccScaledMisalignedImu(IccImu):
    class ImuParameters(IccImu.ImuParameters):
        ...
        def printDetails(self, dest=sys.stdout):
            ...
            print("    M:", file=dest)
            print(
                self.formatIndented("      ", np.array(self.data["gyroscopes"]["M"])),
                file=dest,
            )
            print("    A [(rad/s)/(m/s^2)]:", file=dest)
            print(
                self.formatIndented("      ", np.array(self.data["gyroscopes"]["A"])),
                file=dest,
            )
            print("    C_gyro_i:", file=dest)
            print(
                self.formatIndented(
                    "      ", np.array(self.data["gyroscopes"]["C_gyro_i"])
                ),
                file=dest,
            )
            ...
            print(
                self.formatIndented(
                    "      ", np.array(self.data["accelerometers"]["M"])
                ),
                file=dest,
            )
    ...
    def updateImuConfig(self):
        ...
        self.imuConfig.setIntrisicsMatrices(
            self.M_accel_Dv.toMatrix3x3(),
    ...
    def addDesignVariables(self, problem):
        ...
        self.q_gyro_i_Dv = aopt.RotationQuaternionDv(np.array([0.0, 0.0, 0.0, 1.0]))
        problem.addDesignVariable(self.q_gyro_i_Dv, HELPER_GROUP_ID)
        self.q_gyro_i_Dv.setActive(True)

        self.M_accel_Dv = aopt.MatrixBasicDv(
            np.eye(3), np.array([[1, 0, 0], [1, 1, 0], [1, 1, 1]], dtype=int)
        )
        problem.addDesignVariable(self.M_accel_Dv, HELPER_GROUP_ID)
        self.M_accel_Dv.setActive(True)

        self.M_gyro_Dv = aopt.MatrixBasicDv(
            np.eye(3), np.array([[1, 0, 0], [1, 1, 0], [1, 1, 1]], dtype=int)
        )
        ...
        self.M_accel_gyro_Dv = aopt.MatrixBasicDv(
            np.zeros((3, 3)), np.ones((3, 3), dtype=int)
        )
...
class IccScaledMisalignedSizeEffectImu(IccScaledMisalignedImu):
    ...
    def updateImuConfig(self):
        ...
        self.imuConfig.setAccelerometerLeverArms(
            self.rx_i_Dv.toEuclidean(),
    ...
    def addAccelerometerErrorTerms(
        ...
        for im in self.imuData:
            ...
            if (
                ...
                C_i_b = self.q_i_b_Dv.toExpression()
                rx_b = (
                    self.r_b_Dv.toExpression()
                    + C_i_b.inverse() * self.rx_i_Dv.toExpression()
                )
                ry_b = (
                    self.r_b_Dv.toExpression()
                    + C_i_b.inverse() * self.ry_i_Dv.toExpression()
                )
                rz_b = (
                    self.r_b_Dv.toExpression()
                    + C_i_b.inverse() * self.rz_i_Dv.toExpression()
                )
